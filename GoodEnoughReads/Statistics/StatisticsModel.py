...
class StatisticsModel:
    ...
    def CalculateBookPagesPerYear(self):
        self.cursor.execute("SELECT C.NewestReadingEndDate, B.Pages FROM BookInUserCollection AS C, Book AS B WHERE C.email = ? AND B.ISBN = C.ISBN;", self.email)
    ...
    def CalculateGenresPerYear(self): 
        self.cursor.execute("SELECT C.NewestReadingEndDate, B.Genre FROM BookInUserCollection AS C, Book AS B WHERE C.email = ? AND B.ISBN = C.ISBN;", self.email)
    ...
    def CalcBookReread(self):
        self.cursor.execute("SELECT B.Title, C.NumberOfTimesReread FROM BookInUserCollection AS C, Book AS B WHERE C.email = ? AND B.ISBN = C.ISBN AND C.NumberOfTimesReread > 1;", self.email)
