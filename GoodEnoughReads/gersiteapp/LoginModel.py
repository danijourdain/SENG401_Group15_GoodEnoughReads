...
class LoginModel:
    ...
    def signupUser(self):
        ...
        self.cursor.execute("INSERT INTO User(email, `Name`, ProfilePictureURL, XP, AwardProfile) VALUES (?, ?, NULL, 0, 1);", self.email, name)
        return True
        
    def updateUser(self, name):
        self.cursor.execute("UPDATE `User` SET `Name`= ? WHERE Email = ?;", name, self.email)
